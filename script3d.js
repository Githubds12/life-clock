...
function showSutra(index) {
  ...
  setTimeout(() => {
    ...
    if (sutraCounter) {
      sutraCounter.textContent = `Sutra ${index + 1} of ${sadhguruSutras.length}`;
    }
  }, 600);
...
if (sutrasPrevBtn) {
  sutrasPrevBtn.addEventListener("click", () => {
    stopSutraAutoCycle();
    sutraIndex = (sutraIndex - 1 + sadhguruSutras.length) % sadhguruSutras.length;
    showSutra(sutraIndex);
  });
}
...
function showBookExcerpt(index) {
  ...
  setTimeout(() => {
    ...
    if (bookExcerptsCounter) {
      bookExcerptsCounter.textContent = `Excerpt ${index + 1} of ${bookExcerpts.length}`;
    }
  }, 600);
...
if (bookExcerptsPrevBtn) {
  bookExcerptsPrevBtn.addEventListener("click", () => {
    stopBookExcerptsAutoCycle();
    bookExcerptsIndex = (bookExcerptsIndex - 1 + bookExcerpts.length) % bookExcerpts.length;
    showBookExcerpt(bookExcerptsIndex);
  });
}
