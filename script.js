(() => {
  ...
  function drawStream(topLevelY, bottomLevelY) {
    ...
    // --- Falling stream (thick at top, thin at bottom) ---
    ctx.save();
    const topWidth = 8; // wider at the neck
    const bottomWidth = 2; // very thin near the pile
    const startY = neckY + 2;
    const endY = bottomLevelY - 2;

    const gradient = ctx.createLinearGradient(W / 2, startY, W / 2, endY);
    gradient.addColorStop(0, sandGradA); // brighter at top
    gradient.addColorStop(1, sandGradB); // darker at bottom
  ...
  function updateStats() {
    ...
    if (dobInput) {
      ...
      document.getElementById(
        "exactAge"
      ).textContent = `${exactYears} years, ${months} months, ${leftoverDays} days`;
    }
